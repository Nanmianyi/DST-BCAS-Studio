...
它来自我前些年对AMD的CAS锐化后处理技术的改进开源算法“BCAS”（双边自适应锐化）
核心思想是针对图形资源做超高质量的锐化处理，同时不产生任何可见过充与伪影等效提升画质清晰度，并进行高质量的色彩处理

双边锐化是一种高质量的离线锐化技术，常见于Photoshop，达芬奇等软件，其效果自然但传统形式开销巨大，
...
| 原版 | BCAS Studio |
| --- | --- |
| ![原版2](docs/images/comparison2_before.png) | ![BCAS Studio 2](docs/images/comparison2_after.png) |
...
后处理：没有动态范围管理、没有边缘重建。BCAS Studio 用
...
- **CDL 一级/二级调色**（Color Decision List，ASC 决策清单标准）：
  ...
  可复刻任何调色软件的色彩倾向；
- **高光去饱和**：按亮度权重把过曝区域拉回白色，去除高光上的饱和颜色分离层次；
- **ACES 胶片曲线**：3D渲染与电源非常常用的高质量标准色调映射，sRGB 软底 EOTF 线性化
  （带线性 Toe，8-bit 暗部前 16 级灰阶不坍塌）→ 曲线 → 再编码；
- **科学补偿动态范围**：上述组合把饥荒"平"得发灰的直出画面重新拉出
  高光-中间调-阴影的层次关系，细粒有分，各分己工。

**「画质工作室」面板**——五个页签、40+ 项参数，拖数值微调、点数值键入、
行尾 R 复位，全部实时生效：

![画质工作室面板](docs/images/ui_panel.png)

**白平衡体素调色轮**——色相环无级取色，实时映射为 RGB 增益
（等效 CDL 斜率）：

![白平衡体素调色轮](docs/images/ui_colorwheel.png)

### 2. 双边自适应锐化（Pass 2 · 本 Mod 的画质核心）

对图像资源进行精准重置与改进的自研锐化管线，每一步都有明确的信号处理依据，核心思想是高质量选择性的处理：
...
在开启综上所述的默认配置后，在水中木群系的光影效果与层次得到质的飞跃，
特别是光影层次上此前完全融合在一起，开启后的层次相当分明：

| 原版 | BCAS Studio |
| --- | --- |
| ![海中原版](docs/images/sea_before.png) | ![海中开启后](docs/images/sea_after.png) |

> 水中木群系雨天实机对比：同场景、同机位、同光照。开启前水面光影完全
> 融合成一片，开启后水光斑驳、光束立体、层次分明（作者特调预设）
...
本项目采用 **MIT 协议** 全开源——这也是我把它公开出来的意义：

- **技术意义**：在饥荒联机版里打通一条完整的自定义后处理管线
  （ksh 容器构建、uniform 绑定、双 pass 排序、8-bit 加固），这条路的
  每一步坑我都踩过并留在了文档里。后来者不必再从零摸索，
  任何想给饥荒写后处理效果——无论是锐化、超分还是别的什么——的人，
  都可以直接站在这套管线上开始。
- **算法意义**：BCAS 的双边自适应锐化算法本来就是我开源出来的东西，
  从 ReShade 时代到现在，它欢迎任何形式的移植、改进与再创作。
- **使用意义**：你可以随意复制、修改、二次发布，包括用于你自己的 Mod——
  唯一的请求是保留一份对原作者的署名。

> 附带说明：Mod 内置的思源黑体字体资源版权归其原作者所有，
> 遵循 `BCAS-Studio/fonts/ATTRIBUTION.txt` 中的授权说明，
> 不受本仓库 MIT 协议约束。
