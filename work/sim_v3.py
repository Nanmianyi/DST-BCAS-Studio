...
def caustics(px,pz,t,grain,gain,deep):
    ...
    wxp=px+(wx-f32(0.5))*f32(4.0); wzp=pz+(wz-f32(0.5))*f32(4.0)
    dx,dz=f32(0.86),f32(0.51); pdx,pdz=-dz,dx
    u=wxp*dx+wzp*dz; v=wxp*pdx+wzp*pdz
    su=f32(0.32)*(1-f32(0.35)*deep); sv=f32(0.40)*(1+f32(1.60)*deep)
    qx=u*su*grain; qz=v*sv*grain
    ...
    return cav*g*(1-deep*f32(0.80))
t=f32(6.0); grain=f32(1.0); density=f32(0.93); strength=f32(0.55); soft=f32(0.45)
W=1000; Hh=520; extent=26.0
xs=np.linspace(0.5,0.5+extent,W,dtype=np.float32); zs=np.linspace(0.5,0.5+extent*Hh/W,Hh,dtype=np.float32)
X,Z=np.meshgrid(xs,zs)
open_=np.clip(X/extent,0,1)  # fake: left shore, right open
open_=np.clip(open_+(vnoise(X*f32(0.03),Z*f32(0.03))-f32(0.5))*f32(0.30),0,1)
dd=np.clip((open_-f32(0.20))/f32(0.65),0,1); deep=dd*dd*(3-2*dd)
...
gain=f32(10)
cau=caustics(X,Z,t,grain,gain,deep)
gs=np.clip((deep-f32(0.15))/f32(0.55),0,1); gs=gs*gs*(3-2*gs)
glitcol=np.stack([f32(1.0)*(1-deep)+f32(0.80)*deep, f32(0.96)*(1-deep)+f32(0.95)*deep, f32(0.84)*(1-deep)+f32(1.0)*deep],-1)
col=seabed*(1-gs)[...,None]+glitcol*cau[...,None]
# fake ocean color (deep cyan)
ocean_rgb=np.stack([f32(0.9)-deep*f32(0.55), f32(0.95)-deep*f32(0.25), f32(0.9)+deep*f32(0.05)],-1)
oceanDeep=np.maximum(ocean_rgb,f32(0.10))
mixf=soft*f32(0.5)+deep*f32(0.40)+f32(0.10)
col=col*(1-mixf[...,None])+col*oceanDeep*mixf[...,None]
amt=strength*(1-deep*f32(0.80))*f32(0.90)
base=np.array([0.10,0.30,0.32],dtype=np.float32)
out=np.clip(col*amt[...,None]+base*(1-amt[...,None]),0,1)
Image.fromarray((out*255).astype(np.uint8)).save('work/v3_preview.png')
print('cau max',round(float(cau.max()),2),'shallow cau>0.3',round(float((cau[:, :300]>0.3).mean()),4))
print('done')
