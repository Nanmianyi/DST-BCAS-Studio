...
void main()
{
    ...
    // Camera world basis from the view matrix rows. Column i of a GLSL mat4 is
    // MatrixV[i], so row 0 (the camera right axis) is
    // (MatrixV[0].x, MatrixV[1].x, MatrixV[2].x) and row 2 is the back axis
    // (the engine's GetDownVec convention: scene -> camera is +back).
    //
