...
【公告 · v3.10.1 紧急修复：温泉 / 洞穴退出崩溃】

· 修「启用动态太阳光影，靠近温泉就崩」「调用了一个不存在的水面渲染参数」：
  水面特效的导出名写错了一处 —— 导出表写成自引用一个不存在的字段，于是
  `AttachWater` 取到 nil，温泉（hotspring）一生成就调用它 ⇒ 当场崩在
  `modmain.lua:408: attempt to call field 'AttachWater' (a nil value)`。
  现在导出指向真正的实现（`AttachWaterFX`）。
· 修「从洞穴里出来时（偶尔）崩溃」：光柱实体属于**创建它的那个世界**，而洞穴进出会
  换世界对象 —— 切换后旧引用已经是已销毁实体，绘制时碰 Transform/AnimState/Light 是
  引擎级硬崩。现在换世界会重建光柱、清空旧引用，绘制函数自己也先验一次实体存活。
· 顺带：跨世界状态（调度器标志、水面灯表）在世界切换时一并复位，回地面后光柱与
  水面灯按时重建，不会再带着上个世界的引用跑。

【公告 · 背光冷暖 / 背光黑白（v3.10.0）】立体光照页新增两把**背光专用**滑条

· 「背光冷暖」：往冷蓝 B 通道上调（冷端 B +0.200 / R −0.149），背光面质感更通透；
  不喜欢冷调就往暖端拉，全区间单调、平滑、无跳变。
· 「背光黑白」：单控背光面明暗（0.40~1.60），受光面不受影响。
· 两把**只吃背光面**：受光面提亮 / 边缘光 / 根部接触的数值逐位不变（同源逐位比对
  884736 组，0 处不同）；出厂 0.5 档解出精确 (1,1,1)，画质与上一版完全一致。
· 编码：方位角与两把滑条合并进浮参 x 槽（低 12 位滑条 + 高 12 位方位角，共 24 位），
  已做 2^24 全量回环验证，影长/仰角/强度槽的精度一位未丢。

【公告 · 剪影投影内核重做】影子不再越叠越黑，逆光时整面背光 + 轮廓发光

· 影子改成**镜射施影者自己的绘制**（bank/build/剪辑/帧，含皮肤、骑乘、
  砍伐、生长阶段），以纯色剪影贴地；方向与影长由日晷推导。
· 透明度走**屏幕门**：每片元只输出纯色或什么都不画，
  所以同一施影者的部件再怎么重叠，画面上也只有一层 —— 重叠台阶恒等于 0，
  不会再有"部件组装结构在影子上显形"的斑驳感。抖动图案锚在世界坐标上
  （不会在有纹理的地皮上打出摩尔纹），颗粒细于一个屏幕像素，拉近视距也不会看成方块。
· 物体表面受光面/背光面：**逆光时整面背光、只剩轮廓发光**
  （太阳或火光在物体背后时，受光面正确地消失）。
· 夜晚与洞穴：影子重新开启，方向**完全由附近的光辉源判定**（火光在物体哪侧，
  影子就朝反侧长）；黄昏/黎明的模式切换带滞回与交叉淡变，不会再出现"影子转圈"
  或"突然蹦出来"——天刚亮时不显示，过几秒才慢慢淡进来。
· 性能：对象池 + 矩形裁切，静态施影者进入睡眠，不逐帧重算；
  夜晚无光源时零开销。
...
· 世界空间太阳（日晷模型）：单一解析式输出全局共享的影子长度、
  ...
  太阳第一次在饥荒里走完一天。
· 剪影投影：镜射施影者自己的绘制（动画/皮肤/骑乘/换装/生长阶段自动同步，
  砍伐、燃烧、收割即时反映），以单层纯色剪影落地，屏幕门半透明保证
  **部件重叠不加深**；逆光时物体整面背光、轮廓发光。
· 满月夜月光投影（染月光蓝），雨雪衰减，冬季收敛。
· 性能架构：零影子实体、零逐帧镜像（旧方案的实体与调度器全部下线）；
  三个太阳圆盘射线 × 可变步数在半分辨率上解算，夜晚与洞穴零开销门控。
...
≡ 内置高清字体 ≡
思源黑体（Source Han Sans）85px 视网膜字模（取自创意工坊 Chinese++）：
...
≡ 版本 ≡
v3.9.0：影子观感全面重做 —— 硬边平涂改为带半影的柔影：贴地端最实最锐、越高半影
越宽越淡（树冠/帽子化开），边缘柔化，整块只保留一个颜色并带一点天光冷调。
...
configuration_options = {
    {
        name = "LIGHTING",
        label = "动态太阳光影总开关",
        hover = "一键启用或彻底关闭动态太阳光影系统（剪影投影、物体受光/背光、日晷扫动、透云光束）。影子在「09 影子」页有浓度等参数；关闭后影子系统整体静默、原生投影放行。",
        options = {
    ...
    },
    {
        name = "HDFONT",
        label = "高清字体",
        hover = "内置思源黑体 (85px 视网膜字模，取自创意工坊 Chinese++)。全局生效，含主菜单。关闭只影响游戏界面字体；本模组设置面板始终使用自带高清字体，不受此项与其他字体模组影响。",
        options = {
