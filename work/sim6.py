...
nx/=ln; nz/=ln; ny=1/ln
slope=np.sqrt(nx**2+nz**2)
print('tilt max',round(float(np.degrees(np.arctan(slope.max()))),1),'mean',round(float(np.degrees(np.arctan(slope.mean()))),1))
# sun elevation 45, az 20
D=math.radians
...
for k in (4,7,11,16):
    gl=np.power(ndl,k,dtype=np.float32)
    print(f'k={k} glint>0.3 frac={float((gl>0.3).mean()):.4f} mean={float(gl.mean()):.4f}')
    Image.fromarray((np.clip(gl,0,1)*255).astype(np.uint8)).save(f'work/sim6_ndl_{k}.png')
print('done')
