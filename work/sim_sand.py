...
def sand(px,pz,t):
    m=f32(0.5)+f32(0.5)*(snoise(px*f32(0.16),pz*f32(0.16))*f32(0.80)+snoise(px*f32(0.45)+f32(7.1),pz*f32(0.45)+f32(3.3))*f32(0.40))
    lo=np.array([0.62,0.70,0.47],dtype=np.float32); hi=np.array([0.84,0.85,0.66],dtype=np.float32)
    col=lo[None,None,:]+(hi-lo)[None,None,:]*np.clip(m,0,1)[...,None]
    sp=vnoise(px*f32(3.0),pz*f32(3.0)+t*f32(0.03))
    s=np.clip((sp-f32(0.82))/f32(0.15),0,1); s=s*s*(3-2*s)
    col=col*(1-f32(0.13)*s)[...,None]
    line=np.abs(snoise(px*f32(0.55)+f32(3.7),pz*f32(0.55)+f32(9.2)))
    li=np.clip((f32(0.012)-line)/f32(0.012),0,1); li=li*li*(3-2*li)
    col=col*(1-f32(0.26)*li)[...,None]
    return col
W=800; extent=12.0
xs=np.linspace(0.5,0.5+extent,W,dtype=np.float32); zs=np.linspace(0.5,0.5+extent,W,dtype=np.float32)
X,Z=np.meshgrid(xs,zs)
c=sand(X,Z,f32(4.0))
Image.fromarray((np.clip(c,0,1)*255).astype(np.uint8)).save('work/sim_sandtex.png')
print('sand rgb mean',[round(float(c[...,i].mean()),3) for i in range(3)])
print('done')
